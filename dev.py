...
def lint(args: list[str]) -> None:
    run_subprocess(["ruff", "check", "src", "tests", "analysis"] + args)
    run_subprocess(["mypy", "src", "tests", "analysis", "--install-types", "--non-interactive"] + args)


def format(args: list[str]) -> None:
    run_subprocess(["black", "src", "tests", "analysis"] + args)
    run_subprocess(["ruff", "check", "src", "tests", "analysis", "--fix", "--unsafe-fixes"] + args)
