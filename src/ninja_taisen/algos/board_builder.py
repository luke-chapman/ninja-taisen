...
def make_board(random: SafeRandom, shuffle_cards: bool = True) -> Board:
    return Board(monkey_cards=__monkey_cards(random, shuffle_cards), wolf_cards=__wolf_cards(random, shuffle_cards))


def __monkey_cards(random: SafeRandom, shuffle_cards: bool) -> defaultdict[int, list[Card]]:
    ...
    __add_remaining_cards(cards, remaining_cards, {0: 3, 1: 3, 2: 2, 3: 1})

    return cards


def __wolf_cards(random: SafeRandom, shuffle_cards: bool) -> defaultdict[int, list[Card]]:
    cards: defaultdict[int, list[Card]] = defaultdict(list)
    cards[10].append(WJ4)
    ...
    if shuffle_cards:
        random.shuffle(remaining_cards)
    __add_remaining_cards(cards, remaining_cards, {10: 3, 9: 3, 8: 2, 7: 1})

    return cards


def __add_remaining_cards(
