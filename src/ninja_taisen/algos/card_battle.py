from ninja_taisen.public_types import Card, Category
...
def battle_winner(card_a: Card, card_b: Card) -> Card | None:
    ...
    if card_a.combat_category == Category.joker:
        if card_b.combat_category == Category.joker:
...
def _joker_versus_joker(joker_a: Card, joker_b: Card) -> Card | None:
    assert joker_a.combat_category == Category.joker, f"Expected {joker_a} to be a joker"
    assert joker_b.combat_category == Category.joker, f"Expected {joker_b} to be a joker"
...
def _joker_versus_non_joker(joker: Card, non_joker: Card) -> Card | None:
    assert joker.combat_category == Category.joker, f"Expected {joker} to be a joker"

    result = _strength_winner(joker, non_joker)

    if not result:
        joker.strength = 0
    elif result.combat_category == Category.joker:
...
def _rock_paper_scissors_winner(card_a: Card, card_b: Card) -> Card:
    assert card_a.combat_category != Category.joker, f"Expected {card_a} to not be a joker"
    assert card_b.combat_category != Category.joker, f"Expected {card_b} to not be a joker"
    assert (
